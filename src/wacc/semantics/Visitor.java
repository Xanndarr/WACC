...
public class Visitor extends WACCParserBaseVisitor<Void> {
  ...
  public Void visitExit(ExitContext ctx) {
    ...
    if (!nodeType.equals("int")) {
      System.err
          .println("Exit statements expressions must evaluate to an int.");
    }
  ...
  public Void visitIf(IfContext ctx) {
    ...
    if (!nodeType.equals("bool")) {
      System.err
          .println("If statement expressions must evaluate to bool type.");
    }
  ...
  public Void visitUnaryOpExp(UnaryOpExpContext ctx) {
    ...
    case "!":
      if (!nodeType.equals("bool"))
        System.err.println("The '!' operator only works for boolean types.");
      nodeType = "bool";
      break;
    case "-":
      if (!nodeType.equals("int"))
        System.err.println("The '-' operator only works for integer types.");
      nodeType = "int";
      break;
    case "len":
      // TODO check node type is of array
      if (!nodeType.equals("ARRAYS?!?!"))
        System.err.println("The 'len' operator only works for array types.");
      nodeType = "int";
      break;
    case "ord":
      if (!nodeType.equals("char"))
        System.err.println("The 'ord' operator only works for char types.");
      nodeType = "int";
      break;
    case "chr":
      if (!nodeType.equals("int"))
        System.err.println("The 'chr' operator only works for int types.");
      nodeType = "char";
      break;
    default:
      nodeType = null;
  ...
  public Void visitAssign_rhs(Assign_rhsContext ctx) {
    // TODO When calling functions, check arg types are equal function signature
    // types
